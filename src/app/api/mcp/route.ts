...
function createServer(baseUrl: string, apiKey: string) {
  ...
  server.tool("create_project", "Create a new project", { name: z.string(), key: z.string().describe("2-5 char uppercase key"), description: z.string().optional(), makerMode: z.boolean().optional().describe("Enable maker mode (simpler 3-state workflow: todo/doing/done)") }, async (params) => textResult(await api("/api/projects", { method: "POST", body: JSON.stringify(params) })));
